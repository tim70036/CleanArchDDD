...
class GetConfigService extends IGetConfigService {
  ...
  public async Get(): Promise<ErrOr<GetConfigSTO>> {
    ...
    const announcement = await redisClient.hGet(
      this.maintenanceKey,
      "announcement",
    );
    const ipWhitelist = await redisClient.hGet(
      this.maintenanceKey,
      "ipWhitelist",
    );
    ...
    if (typeof startTime === "undefined")
      return new InvalidDataError("maintenance data in redis ");
    if (typeof announcement === "undefined")
      return new InvalidDataError(
        "maintenance data in redis do not contain announcement",
      );
    if (typeof ipWhitelist === "undefined")
      return new InvalidDataError(
        "maintenance data in redis do not contain ipWhitelist",
      );
    if (typeof status === "undefined")
      return new InvalidDataError(
        "maintenance data in redis do not contain status",
      );
