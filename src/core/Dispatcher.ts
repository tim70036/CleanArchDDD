...
class Dispatcher {
  ...
  public constructor(versionMap: Map<string, Controller>) {
    ...
    if (!isValid) throw new Error("create versionMap fail");
  ...
  public async Dispatch(
    ...
    if (typeof reqVersion !== "string") reqVersion = "";
    ...
    if (typeof matchedController === "undefined") {
      this.logger.error(
        `cannot find matched controller for version[${version}]`,
      );
      return;
    }
  ...
  private GetVersion(reqVersion: string): string {
    ...
    if (typeof floorVersion === "undefined") return minVersion;
