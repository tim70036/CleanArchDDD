...
class WsMessage {
  private static readonly schema = saferJoi.object({
    ...
    srcUid: saferJoi
      .alternatives()
      .try(
        saferJoi.string().uuid({ version: "uuidv1" }),
        saferJoi.string().valid("server"),
      ),
  });
  ...
  public static CreateFromRaw(rawMessage: string, srcUid: string): WsMessage {
    ...
    if (error)
      throw new Error(
        `Failed creating class[${WsMessage.name}] with message[${error.message}]`,
      );
  ...
  public static Create(
    ...
    const { error } = WsMessage.schema.validate({
      eventCode,
      eventData,
      srcUid,
    });
