...
class MaintenanceMaster {
  ...
  private readonly UpdateMaintenanceSetting = async (): Promise<void> => {
    try {
      ...
      if (
        this.ShouldMaintenanceStart(this.startTime) &&
        config.status === MaintenanceStatus.Off.toString()
      ) {
  ...
  private ShouldMaintenanceStart(startTime: dayjs.Dayjs): boolean {
    const now = dayjs().utc();
    const milisecSinceStartTime = dayjs
      .duration(now.diff(startTime))
      .asMilliseconds();

    return (
      milisecSinceStartTime >= 0 &&
      milisecSinceStartTime < this.refreshInterval * 2
    );
  }
}
