...
class WsApp {
  ...
  public async Init(): Promise<void> {
    ...
    this.wss.on("error", (error) =>
      this.logger.error(`wss on error:`, { error: error }),
    );
  ...
  public readonly HandleUpgrade = async (
    ...
    try {
      ...
      if (sessionOrError.IsFailure()) {
        ...
        if (sessionOrError instanceof DuplicatedError) {
          socket.write(
            `HTTP/1.1 ${StatusCode.PreconditionFailed} ${StatusCode[StatusCode.PreconditionFailed]}\r\n\r\n`,
          );
          socket.destroy();
          return;
        }
        ...
        socket.write(
          `HTTP/1.1 ${StatusCode.Unauthorized} ${StatusCode[StatusCode.Unauthorized]}\r\n\r\n`,
        );
        socket.destroy();
        return;
      }
      ...
      socket.write(
        `HTTP/1.1 ${StatusCode.InternalServerError} ${StatusCode[StatusCode.InternalServerError]}\r\n\r\n`,
      );
      socket.destroy();
      return;
    }
  ...
  protected readonly HandleNewConnection = async (
    ws: SmartSocket,
    uid: string,
  ): Promise<void> => {
    ...
    if (this.wsMap.has(uid)) {
      ...
      oldWs?.close(
        1000,
        `this connection is replaced by a new connection from the same user.`,
      );
    }
    ...
    ws.on("close", async (code, reason) => {
      ...
      if (!this.wsMap.has(uid)) {
        await redisSubClient.unsubscribe(
          `${WsController.userChannelPrefix}${uid}`,
        );
    ...
    ws.on("error", (error) =>
      this.logger.error(`on error`, { uid: uid, error: error }),
    );

    // Send maintenance news if needed.
    const minsTillMaintenance = dayjs
      .duration(maintenanceMaster.StartTime.diff(dayjs.utc()))
      .asMinutes();
    if (minsTillMaintenance > 0 && minsTillMaintenance <= 30) {
      const newsServerEvent = NewsServerWsEvent.Create(
        `伺服器將在 ${Math.floor(minsTillMaintenance)} 分鐘後進行維修`,
      );
      const message = WsMessage.Create(
        NewsServerWsEvent.code,
        newsServerEvent,
        "server",
      );
  ...
  protected readonly IsBlockedByMaintenance = (ip: string): boolean => {
    ...
    if (
      maintenanceMaster.Status === MaintenanceStatus.AllowWhitelist &&
      !maintenanceMaster.IpWhitelist.includes(ip)
    )
      return true;
  ...
  protected readonly ShouldSendMaintenanceNews = (): boolean => {
    ...
    for (const minsBeforeMaintenance of [30, 15, 10, 5]) {
      const sendNewsTime = maintenanceMaster.StartTime.subtract(
        minsBeforeMaintenance,
        "minutes",
      );
      const milisecTillSendNews = dayjs
        .duration(sendNewsTime.diff(now))
        .asMilliseconds();
      ...
      if (
        milisecTillSendNews >= 0 &&
        milisecTillSendNews < WsApp.checkMaintenanceInterval
      )
        return true;
    }
