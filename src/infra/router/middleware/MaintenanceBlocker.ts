...
function MaintenanceBlocker(
  ...
  if (
    (status === MaintenanceStatus.BlockAll &&
      allowDomainsWhenBlockAll.includes(apiDomain)) ||
    (status === MaintenanceStatus.AllowWhitelist &&
      allowDomainsWhenWhitelist.includes(apiDomain)) ||
    (status === MaintenanceStatus.AllowWhitelist &&
      maintenanceMaster.IpWhitelist.includes(req.ip ?? ""))
  ) {
    next();
    return;
  }
  ...
  res
    ...
    .send(maintenanceMaster.Announcement);
}
