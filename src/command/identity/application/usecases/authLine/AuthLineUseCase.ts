...
class AuthLineUseCase extends UseCase<AuthLineCTO, Session> {
  ...
  protected async Run(request: AuthLineCTO): Promise<ErrOr<Session>> {
    ...
    if (lineIdOrError.IsFailure()) return lineIdOrError;
    ...
    try {
      ...
      if (userOrError.IsSuccess()) {
        ...
        const lineAuthOrError = LineAuth.Create({
          lineId: lineId,
          isValid: true,
        });
        if (lineAuthOrError.IsFailure()) return lineAuthOrError;
    ...
    try {
      ...
      if (verifyResponse.status !== 200)
        return new NotAuthenticatedError(
          `line verify api failed accessToken[${accessToken}] statusCode[${verifyResponse.status}]`,
        );
      ...
      if (profileResponse.status !== 200)
        return new NotAuthenticatedError(
          `line profile api failed accessToken[${accessToken}] statusCode[${profileResponse.status}]`,
        );
