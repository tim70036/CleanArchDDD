    ...
    try {
      ...
      if (
        ...
        return new InvalidDataError(
          `invalid token[${token}] error[${error.name} ${error.message}]`,
        );
      else if (error instanceof jwt.TokenExpiredError)
        return new ExpireError(
          `expired token[${token}] error[${error.name} ${error.message}]`,
        );
    ...
    try {
      ...
      if (token !== session.props.jwt)
        return new DuplicatedError(
          `session invalidated by newer session uid[${rawUid}]`,
        );
