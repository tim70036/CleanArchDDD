...
class UserRepo extends IUserRepo {
  ...
  public Exists(id: EntityId, trx?: Transaction  ): Promise<boolean> {
  ...
  public async GetByDeviceId(deviceId: string): Promise<ErrOr<User>> {
    ...
    if (result.length <= 0)
      return new NotExistError(
        `cannot find deviceId[${deviceId}] in table[${DeviceAuthModel.tableName}]`,
      );

    const uidOrError = EntityId.CreateFrom(fromBinaryUUID(result[0].uid));
    if (uidOrError.IsFailure()) return uidOrError;
  ...
  public async GetByLineId(lineId: string): Promise<ErrOr<User>> {
    ...
    if (result.length <= 0)
      return new NotExistError(
        `cannot find lineId[${lineId}] in table[${LineAuthModel.tableName}]`,
      );
  ...
  public async Save(user: User, trx: Transaction): Promise<void> {
    ...
    await DeviceAuthModel.query(trx.Raw)
      .insert({
        uid: toBinaryUUID(user.id.Value),
    ...
    await LineAuthModel.query(trx.Raw)
      .insert({
        uid: toBinaryUUID(user.id.Value),
