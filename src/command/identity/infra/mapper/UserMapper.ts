...
class UserMapper {
  public ToDomain(dto: {
    ...
    const deviceAuthOrError = DeviceAuth.Create({
      deviceId: dto.deviceAuthDTO.deviceId,
      isValid: dto.deviceAuthDTO.isValid === 1,
    });
    const lineAuthOrError = LineAuth.Create({
      lineId: dto.lineAuthDTO.lineId,
      isValid: dto.lineAuthDTO.isValid === 1,
    });
    ...
    if (dtoResult.IsFailure()) return dtoResult;
